...
fun EditionPage(
    ...
    var name by remember { mutableStateOf(notation.name) }
    var description by remember { mutableStateOf(notation.description) }
    var selectedDate by remember { mutableStateOf<Long?>(notation.setTo) }
    var selectedTimeMinutes by remember {
        mutableStateOf<Int?>(
            Date(notation.setTo).minutes + Date(
                notation.setTo
            ).hours * 60
        )
    }
    ...
    Box(
        ...
        Column(
            ...
            Row(
                ...
                Column {
                    ...
                    if (selectedDate == null) {
                        Text("Select a date")
                    } else if (selectedTimeMinutes == null) {
                        Text("Select a time")
                    } else {
                        val date =
                            dateTimeFormater.combineDateAndTime(
                                selectedDate ?: 0,
                                selectedTimeMinutes ?: 0
                            )
                        Column { Text(dateTimeFormater.formatDateToString(date)) }
                        Column { Text(dateTimeFormater.formatTimeToString(date)) }
                    }
                }
                Button(
            ...
            }, modifier = Modifier.align(Alignment.CenterHorizontally)) { Text("Edit notation") }
        }
    }
